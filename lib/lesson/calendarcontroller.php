...
class CalendarController {
	...
	static function displayCalendar() {
		...
		$calendarId = F3::get('PARAMS["calendarId"]') ? F3::get('PARAMS["calendarId"]') : 0;

		if(Calendar::verifyCalendarId($calendarId)) {
			F3::set('lessons', Lesson::getLessonsForCalendar($calendarId));
	...
	*/
	...
	static function editCalendar ($id) {
		if($_POST['calendar_title']) {
			...
			CalendarController::fillForm($id);
	...
	static function deleteCalendar($id) {
		if(Calendar::verifyCalendarId() && Help::deleteConfirmed()) {
 		}
	}
}
